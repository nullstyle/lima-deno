 ...
 * subpath's `FakeLimactl` — and assert the exact argv sequence WITHOUT a real
 * VM or a `limactl` binary on PATH.
 *
 * A runner never throws on a nonzero exit: some probes (e.g. `test -e`) treat
 * failure as data. Callers that require success use {@linkcode runChecked},
 * which throws a typed {@linkcode CommandError}.
 *
 * @example Run a command and capture its output
 * ```ts
 * import { DenoCommandRunner, runChecked } from "@nullstyle/lima/runner";
 *
 * const runner = new DenoCommandRunner();
 * const result = await runChecked(runner, "limactl", ["--version"]);
...
export interface RunOptions {
  /** Bytes piped to the child's stdin (e.g. a rendered Lima template). */
  readonly stdin?: string;
  /**
   * Skip the {@linkcode MAX_CAPTURE_BYTES} cap on captured output. The cap
   * defends against unbounded guest output; a few commands legitimately
   * produce large output that must be captured whole. @default false
   */
